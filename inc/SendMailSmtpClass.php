...
class SendMailSmtpClass {
    ...
    */
    function SendMail($em, $theme, $message)
    {
    	...
    	$headers .= "From: Уведомления с сайта ".$idn->decode($_SERVER['HTTP_HOST'])."<$this->smtp_username>\r\n"; // от кого письмо
    	$headers .= "To: <$em>\r\n"; // от кого письмо
    	$headers .= "Subject: $theme\r\n"; // заголовок "тема сообщения"
    	$result =  $this->send($em, $theme, $message, $headers); // отправляем письмо
    }
