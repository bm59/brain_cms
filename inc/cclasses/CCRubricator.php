...
class CCRubricator extends VirtualContent
{
	...
	public $sqlstr;
        ...
        function seo_update()
        {
        	    $q = msq("SELECT * FROM `".$this->getSetting('table')."` WHERE `ptitle`=''");
                while ($r = msr($q))
                {
                	msq("UPDATE `".$this->getSetting('table')."` SET
                	`pseudolink`='".get_url_text($r['name'])."',
                    `ptitle`='".$r['name']." купить в Перми',
                    `pdescription`='".$r['name']." купить в Перми'
        ...
        function save(){
                ...
                if ($this->getSetting('table')=='site_site_prubricator_rubricator_13')	$this->seo_update();
                ...
                if (isset($_POST['showsave'])){
					foreach ($_POST as $k=>$v){
						if (preg_match('|^prec\_[0-9]+$|',$k)){
							...
							msq("UPDATE `".$this->getSetting('table')."` SET `show`='".(($_POST['checkshow'.$p]=='on' || !isset($_POST['checkshow'.$p]))?'1':'0')."',`precedence`='".floor($_POST['prec_'.$p])."' WHERE `id`='$p'");
                                        ...
                                        foreach ($list as $pub){
                                                ...
                                                <tr>
                                                        ...
                                                        <td class="t_left">
                                                        	...
                                                        	<div style="color:#666666"><?=$pub['pseudolink']?></div>
                                                        	<div>
                                                        		<table class="table-content stat">
                                                        		<?
