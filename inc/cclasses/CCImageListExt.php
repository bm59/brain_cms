...
class CCImageListExt extends VirtualContent
{
        ...
        function getList($page = 1,$usl = array(),$ord='', $searchnumgood='', $searchtextgood=''){
                ...
                $this->setSetting('page',$page);
 				$q = msq("SELECT * FROM `".$this->getSetting('table')."`".$conditions." ORDER BY `id` DESC LIMIT ".(($page-1)*$this->getSetting('onpage')).",".$this->getSetting('onpage'));
        ...
        function save(){
                ...
                foreach ($_POST as $k=>$v)
		        {
		            ...
		            if (preg_match('|^rub\_[0-9]+$|',$k) && $v=='on')
		            if (preg_replace('|^rub\_([0-9]+)$|','\\1',$k)!=$subsec_id)
		            {
		                     $rub_ids.=preg_replace('|^rub\_([0-9]+)$|','\\1',$k).',';
		            }

		            $_POST[$k]=mysql_real_escape_string($v);
		        }
		        if ($rub_ids!='') $rub_ids=','.$rub_ids;
                                ...
                                foreach ($dataset['types'] as $dt)
                                {
                                        ...
                                        if ($dt['name']=='dopinfo' && $_GET['section']==25)
                                        {
                                        	...
                                        	<div style="padding: 10px 0;">
                                        		Выберите одну или несколько рубрик:
                                        		<?

                                        		$rubrics=msq("SELECT * FROM `site_site_prubricator_rubricator_29` ORDER BY `name`");
                                        		while ($rub=msr($rubrics))
                                        		{
                                        			$k=$rub['id'];
                                        			$v=$rub['name'];
                                        			print '<div><label><input style="margin-right: 3px;" type="checkbox" name="rub_'.$k.'" id="rub_'.$k.'" '.(($_POST['rub_'.$k]=='on' || stripos($pub['rubrics'], ','.$k.',')!==false) ? ' checked="CHECKED"' : '').'/>'.$v.'</label></div>';
                                        ...
                                        foreach ($list as $pub){
                                        	$image=$Storage->getFile($pub['image']);
                                                ...
                                                <tr>
                                                        ...
                                                        <?if ($image['path']=='') {?><a href="./?section=<?=$section['id']?>&pub=<?=$pub['id']?>">редактировать</a><?}?>
                                                        </td>
                                                        <td class="t_32width"><a href="./?section=<?=$section['id']?>&pub=<?=$pub['id']?>">
