...
class CDFLOAT extends VirtualType
{
	...
	function drawEditor($divstyle = '',$span = true){
		...
		<div class="place" <?=($divstyle!='')?$divstyle:''?>>
			...
			<span class="input">
				<input type="text" <?=$selection?> <?if ($this->getSetting('name')=='filelink') print "id=".htmlspecialchars($this->getSetting('name')) ?> name="<?=htmlspecialchars($this->getSetting('name'))?>"  maxlength="<?=$this->getSetting('maxlength')?>" value="<?=stripslashes($this->getSetting('value'))?>" />
			</span>
		</div>
		<?
		if ($span) print '
