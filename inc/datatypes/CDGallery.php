...
class CDGallery extends VirtualType
{
	...
	function drawEditor($divstyle = '',$span = true){
        ...
        {
       		if (!confirm('¬ы уверены, что хотите удалить этот файл?')) return false;
  ...
  $(function() {
    ...
    //$( "#sortable" ).disableSelection();
			...
			<?
			/*if ($this->getSetting('name')=='image' && $_GET['section']==7) 	print '<div style="padding: 0 5px; color: #ff0000">–екомендуемый размер:  1040x450 пикселей</div>';*/
			?>
               ...
               <span class="clear"></span>
				...
				{
					$img_descr='<small>–екомендуемый размер изображени€:';
					...
					print $img_descr.' ≈сли размеры загружаемого изображени€ больше - его размеры будут автоматически изменены</small>';
				}
				?>
				...
				if ($img>0)
				{                 	$image = $Storage->getFile($img);
					...
					<span class="button txtstyle"><input type="button" onclick="delete_file_image<?=htmlspecialchars($this->getSetting('name'))?>(this); return false" id="delete_button_image" style="background-image: url(/pics/editor/delete.gif)" title="”далить изображение"/></span>
	...
	function preSave(){
		...
		$errors = array();
		$settings = $this->getSetting('settings');
		if ((isset($settings['important'])) && ($newvalue<1)) $errors[] = '«аполните поле Ђ'.$this->getSetting('description').'ї';
