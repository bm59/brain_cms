...
class CDINTEGER extends VirtualType
{
	...
	function drawEditor($divstyle = '',$span = true){
		...
		<div class="place" <?=($divstyle!='')?$divstyle:''?>>
			...
			<span class="input">
				<span class="bl"></span>
				<span class="bc"><input type="text" <?=$selection?> <?if ($this->getSetting('name')=='filelink') print "id=".htmlspecialchars($this->getSetting('name')) ?> name="<?=htmlspecialchars($this->getSetting('name'))?>"  maxlength="<?=$this->getSetting('maxlength')?>" value="<?=stripslashes($this->getSetting('value'))?>" /></span>
				<span class="br"></span>
			</span>
		</div>
		<?
