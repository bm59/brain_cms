...
function apply(file,external){
	...
	if($.inArray(ext, ext_video) > -1){
	    fill='<video controls source src="'+path+file+'" type="video/'+ext+'">'+alt_name+'</video>';
	}else{
	    if($.inArray(ext, ext_audio) > -1 ){
		if (ext=='mp3') { ext='mpeg'; }
		fill='<audio controls src="'+path+file+'" type="audio/'+ext+'">'+alt_name+'</audio>';
	    }else{
		fill='<p><a href="'+path+file+'" title="'+alt_name+'" type="file/'+ext+'">'+alt_name+'</a></p>';
	    }
	}
