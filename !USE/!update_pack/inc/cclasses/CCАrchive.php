...
Функционал Новостей
*/
class CCArchive extends VirtualContent
{
  	public $urlstr;
	public $sqlstr;
		...
		function init($settings){
                ...
                if ($this->searchtext!='')
				($this->sqlstr =='') ? $this->sqlstr.=" where `header` LIKE '%".addslashes($this->searchtext)."%'" : $this->sqlstr.=" and `header` LIKE '%".addslashes($this->searchtext)."%'";
     	}
        function getList($searchtext, $page = 1, $orderby=''){
                ...
                $count = floor($count['c']);
                $page = floor($page);
                if ($orderby=='') $orderby='`year` DESC, `num` DESC';
        ...
        function start(){
                if (isset($_GET['pub'])){
                        ...
                        $pdf_filestorage = $this->getSetting('pdf_filestorage');

                        foreach ($dataset['types'] as $k=>$dt){
                                $tface = new $dt['type'];
                                $tface->init(array('name'=>$dt['name'],'description'=>$dt['description'],'imagestorage'=>floor($imagestorage['id']),'filestorage'=>floor($filestorage['id']),'pdf_filestorage'=>floor($pdf_filestorage['id']),'theme'=>$dataset['name'].'_'.$this->getSetting('section'),'rubric'=>$dt['name'],'settings'=>$dt['settings']));
                ...
                <div id="content" class="forms">
                        ...
                        <form id="editform" name="editform" action="<?=$_SERVER['REQUEST_URI']?>" method="POST" enctype="multipart/form-data">
                                ...
                                foreach ($dataset['types'] as $dt){
                                        $tface = $dt['face'];
                                        $stylearray = array(
                                                "date"=>'style="width:18%; margin-right:2%;"',
                                                "num"=>'style="width:32%; margin-right:2%;"',
                                                "year"=>'style="width:32%;"',
                                                "pseudolink"=>'style="width:32%;"'
                                        );
                                        $nospans = array("date","num","pdescription");

                                        if ($dt['name']!='year')
                                        $tface->drawEditor($stylearray[$tface->getSetting('name')],((in_array($tface->getSetting('name'),$nospans))?false:true));
                                        else
                                        {                                        	$year_array=array(''=>'&nbsp');

                                        	for ($i=date('Y'); $i>=2011; $i--)
                                        	$year_array[$i]=$i;

                                        	print
		                                          '<div class="place" id="year" style="z-index: 15; width:32%;">
														<label>'.$dt['description'].(($dt['settings']['important']==1) ? ' <span class="important">*</span>' : '').'</label>';
														print getSelectSinonim('year',$year_array,$tface->getSetting('value'));
											print '</div><span class="clear"></span>';
                                        }
                                }
                        ?>
                        <div class="place">
        ...
        function SetPseudolink()
		{
			...
			while ($r = msr($q))
			{
				msq("UPDATE `".$this->getSetting('table')."` SET pseudolink='".get_url_text($r['header'])."' WHERE id=".$r['id']);
			}
 ...
 <!--//                       <form id="searchform" name="searchform" action="./?section=<?=$section['id']?>" method="POST">
                                ...
                                <form id="showsave" class="showsave" name="showsave" action="./?section=<?=$section['id']?><?=($this->getSetting('page')>1)?'&page='.$this->getSetting('page'):''?>" method="POST">
                                        <table class="table-content stat">
                                                <tr>
                                                        ...
                                                        <th class="t_nowrap">Номер</th>
                                                        <th class="t_nowrap">Год</th>
                                                        <th class="t_32width"></th>
                                                </tr>
                                        <?
                                        foreach ($list as $pub){
                                                ...
                                                <tr>
                                                        ...
                                                        <td class="t_32width"><?=$MySqlConnect->dateFromDBDot($pub['date'])?></td>
                                                        <td class="t_left"><a href="./?section=<?=$section['id']?>&pub=<?=$pub['id']?>"><?=htmlspecialchars($pub['num'])?></a></td>
                                                        <td class="t_left"><?=htmlspecialchars($pub['year'])?></td>
                                                        <td class="t_32width">
        ...
        function deletePub($id){
                ...
                if ($r = msr(msq("SELECT * FROM `".$this->getSetting('table')."` WHERE `id`='".$id."'"))){
                        ...
                        foreach ($dataset['types'] as $dt){
                                $tface = new $dt['type'];
                                $tface->init(array('name'=>$dt['name'],'description'=>$dt['description'],'value'=>$r[$dt['name']],'imagestorage'=>floor($imagestorage['id']),'filestorage'=>floor($filestorage['id']),'pdf_filestorage'=>floor($pdf_filestorage['id']),'theme'=>$dataset['name'].'_'.$this->getSetting('section'),'rubric'=>$dt['name'],'uid'=>floor($r['id']),'settings'=>$dt['settings']));
                                $tface->delete();
                        }
