...
Функционал Новостей
*/
class CCNewsCat extends VirtualContent
{
  	public $urlstr;
	public $sqlstr;
        ...
        function getList($searchtext, $page = 1, $orderby=''){
                ...
                $count = floor($count['c']);
                $page = floor($page);
                if ($orderby=='') $orderby='`precedence` ASC, `date` ASC';
                ...
                <div id="content" class="forms">
                        ...
                        <form id="editform" name="editform" action="<?=$_SERVER['REQUEST_URI']?>" method="POST" enctype="multipart/form-data">
                                ...
                                foreach ($dataset['types'] as $dt){
                                	    ...
                                	    if ($dt['name']=='cat_id')
                                        {
                                        	$categs=getSprValuesOrder($this->getSetting('catspr'), ' ORDER BY `precedence`');
                                        	print
        ...
        function SetPseudolink()
		{
			...
			while ($r = msr($q))
			{
				...
				elseif (detectlanguage($r['pseudolink'])=='ru')
				msq("UPDATE `".$this->getSetting('table')."` SET pseudolink='".get_url_text($r['pseudolink'])."' WHERE id=".$r['id']);
			}
                                ...
                                <form id="showsave" class="showsave" name="showsave" action="./?section=<?=$section['id']?><?=($this->getSetting('page')>1)?'&page='.$this->getSetting('page'):''?>" method="POST">
                                        <table class="table-content stat">
                                                <tr>
                                                        ...
                                                        <th class="t_nowrap">Категория</th>
                                                        <th class="t_nowrap">Псеводоним</th>
                                                        <th class="t_32width"></th>
                                                </tr>
                                        <?
                                        foreach ($list as $pub){
                                                ...
                                                <tr>
                                                        ...
                                                        <td class="t_left"><a href="./?section=<?=$section['id']?>&pub=<?=$pub['id']?>"><?=htmlspecialchars($pub['header'])?></a></td>
                                                        <td class="t_32width"><?=$categs[$pub['cat_id']]?></td>
